...
const { priceListForTransferComponent, PriceListGetter } = usePriceListComposable(
  company_id.value
);
...
const handleChange = (
  ...
  nextTargetKeys.map((id: number) => {
    PriceListGetter.value.map((pl: PriceList) => {
      ...
      if (pl.value === id) {
        productStore.selectedPriceList.push(
          `${pl.label} - Gcia. ${pl.profit_percentage} %`
        );
      }
    });
  });
    ...
    >
      <a-row justify="space-between" align="middle" :gutter="31">
        ...
        <a-col :span="19">
          <a-form-item
            ref="price_list"
            label="Seleccionar listas de precio"
            name="price_list"
          >
            <a-transfer
