class NumbersToWords {
	...
	Seccion(num: number, divisor: number, strSingular: any, strPlural: any) {
	...
	NumeroALetras(num: number, currency: any) {
		const $this = this;
		currency = currency || {};
		const data = {
			numero: num,
		...
		if (data.centavos > 0) {
			data.letrasCentavos =
				...
				(function () {
					...
					else return $this.Millones(data.centavos) + ' ' + data.letrasMonedaCentavoPlural;
				})();
		}
		...
		else return this.Millones(data.enteros) + ' ' + data.letrasMonedaPlural + ' ' + data.letrasCentavos;
	}
}
