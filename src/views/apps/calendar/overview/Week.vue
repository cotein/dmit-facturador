                ...
                <div class="calender-head__navigation">
                    ...
                    <span class="date-label">
                        ...
                        <template
                            v-if="
                                parseInt(
                                    dayjs(dayjs().day('Sunday').year(year).week(currentWeek).toDate()).format('DD'),
                                    10,
                                ) +
                                    6 <=
                            ...
                            "
                            >{{
                                dayjs(dayjs().day('Sunday').year(year).week(currentWeek).toDate()).format('MMM')
                            }}</template
                        ><template v-else>{{
                            dayjs(
                                dayjs()
                ...
                <tr v-for="time in eventTimes" :key="time">
                    ...
                    <td
                        ...
                        :class="`ant-picker-calendar-date-content ${
                            dayjs().format('h A') === time && dayjs().format('MM/DD/YYYY') === day
                                ? 'current-data'
                                : null
                        }`"
                        :title="day"
        ...
        const daysOfWeek = () => {
            ...
            if (minimumDate() + 6 <= daysOfMonth) {
                for (let i = minimumDate(); i <= maximumDate(); i += 1) {
                    days.push(
                        `${dayjs(dayjs().day('Sunday').year(year.value).week(currentWeek.value).toDate()).format(
                            'MM',
                        )}/${i}/${year.value}`,
                    );
                }
